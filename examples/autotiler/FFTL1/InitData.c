...
void InitData4_float(float *Data, int Size, int F1, int F2, int F3, int F4, float p1, float p2, float p3, float p4)
{
	int i;
...
int main(int argc, char *argv[])
{
	...
	for (int i=0; i<MAXDIM; i++) fprintf(Fi, "%d, ", InDataQ16[i]);
	fprintf(Fi, "\n};\n");
	fprintf(Fi, "float InDataf32[] = {\n\t");
	for (int i=0; i<MAXDIM; i++) fprintf(Fi, "%f, ", InDataf32[i]);
	fprintf(Fi, "\n};\n");
	fclose(Fi);
    return 0;
}
