...
def np_fastlog2(x, dtype=np.float32):
    L, B, int_dtype, int_fdtype = FASTLOG_DTYPE_PARAMS[dtype]
    x_f = np.atleast_1d(x).astype(dtype)
    vx = x_f.view(int_dtype)
    mx = ((vx & ((int_dtype(1)<<int_dtype(L))-int_dtype(1))) | (int_dtype(B-1) << int_dtype(L)))
    mxf = mx.view(dtype).astype(int_fdtype)
    y = vx.astype(int_fdtype) / int_fdtype(1 << L)
    return (y + int_fdtype(2.77455363) - int_fdtype(B) - int_fdtype(1.498030302) * mxf - int_fdtype(1.72587999) / (int_fdtype(0.3520887068) + mxf)).astype(dtype)
...
def np_fastpow2(p, dtype=np.float32, out_dtype=None):
    ...
    clipp = np.minimum(np.maximum(p_f, -dtype(clip_val)), dtype(clip_val)) # clip needs to be worked out for each
    z = int_fdtype(clipp - clipp.astype(np.int16 if int_len == 16 else np.int32) + offset) # x - floor(x)
    a = clipp + int_fdtype(B - 5.7259162)
    b = int_fdtype(27.7280233) / (int_fdtype(4.84252568) - z)
    c = - int_fdtype(1.49012907) * z
    c = (int_fdtype(1 << L) * (a+b+c)).astype(np.uint16 if int_len == 16 else np.uint32)
...
def np_fasttanh(p, dtype=np.float32, doalt=True):
    p_f = np.atleast_1d(p).astype(dtype)
    int_fdtype = np.float16 if dtype == bfloat16 and doalt else dtype
    return (int_fdtype(-1.0) + int_fdtype(2.0) / (int_fdtype(1.0) + np_fastexp(dtype(-2.0) * p_f, dtype=dtype, out_dtype=int_fdtype))).astype(dtype)

def np_fastsigmoid(p, dtype=np.float32, doalt=True):
    p_f = np.atleast_1d(p).astype(dtype)
    int_fdtype = np.float16 if dtype == bfloat16 and doalt else dtype
    return (int_fdtype(1.0)/ (int_fdtype(1.0) + np_fastexp(-p_f, dtype=dtype, out_dtype=int_fdtype))).astype(dtype)
