...
@generation_function("kernels", (MatrixAddParameters, ActivationFusion), qrec_types=(QREC_FLOAT, ))
def matadd_kernel_generator_fp16(gen, node, qrec, in_eparams, out_eparams, cname):
...
def gen_mat_add_fp16(code_block, cname, ctrl, feat, width, height, act_oper):
    code_block.write(f'CNN_MatAddAct_fp16("{cname}", {ctrl}, {feat}, {feat}, {width}, {height}, KOP_MATADD, {act_oper});')
...
class MatAddKernel(AutotilerKernel):
    ...
    def code(self, code_block=None):
        ...
        gen_mat_add_fp16(code_block, self.cname, gen_ctrl, self.feat_dim,
