...
class Rfft2DSymmetric(KernelBase):
    @classmethod
    def execute(cls, params,
        ...
        if params.preemp_factor:
            # need preemp here
            in_data = in_data
            max_in = np.max(in_tensors[0])
            qfft_in = qrec.in_qs[0].q
            shift = int(qfft_in - np.ceil(np.log2(max_in)))
            if shift > 0:
        ...
        if params.win_fn:
            ...
            in_data = at_norm(np.multiply(in_data, win_lut, dtype=np.int32), qrec.in_qs[1].q)
        ...
        out_cfft = SwapSamples(out_cfft, swap_table)
...
@qrec_type('symmetric')
class Mfcc2DSymmetric(KernelBase):
    @classmethod
    def execute(cls, params,
                in_tensors,
                qrec: QRec,
