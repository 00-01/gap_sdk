...
def compute_prenorm(qrec, params: FilterParameters):
    if not qrec.cache.get('enable_prenorm'):
        return 0
    max_bits = qrec.in_qs[0].bits - 1 + qrec.in_qs[1].bits - 1 + 1 + \
        math.ceil(math.log2(params.filter.in_c *
...
def gen_mul_biases(qrec, params: MultiplicativeBiasParameters) -> np.ndarray:
    mul_biases_q = qrec.cache.get('mul_biases_q')
...
def apply_multiplicative_bias(qrec, params: FilterParameters, input_tensor: np.ndarray,
                              axis: int, ktype: str = None):
    if ktype == 'float':
        if hasattr(params, 'has_mul_bias') and params.has_mul_bias:
            shape = [params.filter.out_c if idx ==
...
def apply_zero_offset_bias(qrec: QRec, params: Parameters,
