...
/* Small Activation functions inlined */
...
#define APPROX_LUT_RNN
	...
	static inline F16 Fast_Sigmoid_fp16(F16 x)
	{
		return (F16) fsigmoid( (float32_t) x);
	}

	#include "math.h"
	static inline F16 Fast_Tanh_fp16(F16 x)
	{
	...
	static inline float Fast_Sigmoid_fp32(float x_f32){
	    float result;
	...
	static inline F16 Fast_Sigmoid_fp16(F16 x_f16){
	    F16 result;
	...
	static inline F16 Fast_Tanh_fp16(F16 x_f16){
	    F16 result;
	...
	static inline float Fast_Tanh_fp32(float x_f32){
	    float result;
	...
	static inline float Fast_Sigmoid_fp32(float x)
	{
		return ((float)0.5f * (x / ((float)1.0f + AbsF(x))) + (float)0.5f);
	}

	static inline float Fast_Tanh_fp32(float x)
	{
        return (((float)2.0 * x) / ((float) 1.0f + AbsF((float)2.0 * x)));
	}

#endif
#endif

static inline F16 Fast_Swish_fp16(F16 x)
