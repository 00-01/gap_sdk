...
{        
        ...
        if (LogOffset) {
                for (i=First;i<Last;i++){
                        frameOut[i] = (F16_DSP) logf((float) frameIn[i] + LogOffset);
                }
        } else {
                for (i=First;i<Last;i++){
                        frameOut[i] = (F16_DSP) logf((float) (frameIn[i]>1e-6?frameIn[i]:1e-6));
                }
        }
        gap_waitbarrier(0);
...
void MFCC_ComputeLog_f32(MFCC_LogF_T *Arg) 
{
        ...
        if (LogOffset) {
                for (i=First;i<Last;i++){
                        frameOut[i] = (float) logf((float) frameIn[i] + LogOffset);
                }
        } else {
                for (i=First;i<Last;i++){
                        frameOut[i] = (float) logf((float) (frameIn[i]>1e-6?frameIn[i]:1e-6));
                }
        }
        gap_waitbarrier(0);
