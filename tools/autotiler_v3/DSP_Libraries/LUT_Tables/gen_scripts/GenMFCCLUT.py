...
def main():
	...
	Out_str += array_to_def_c_file(SwapTableR2.astype(np.int16), f"SwapTableR2{name_suffix}", "short int", n_fft_int, elem_in_rows=2)
	if round(math.log(n_fft_int, 4)) == math.log(n_fft_int, 4):
		Out_str += array_to_def_c_file(SwapTableR4.astype(np.int16), f"SwapTableR4{name_suffix}", "short int", n_fft_int, elem_in_rows=2)
